...
/** Persistent top bar: app identity + a small API/auth configuration indicator. */
    ...
    <header className="sticky top-0 z-20 border-b border-slate-800 bg-slate-950/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3">
        <Link to="/opportunities" className="flex items-center gap-2">
          ...
          <span className="text-sm text-slate-500">· Operator Console</span>
        </Link>

        <div className="flex items-center gap-2 text-xs">
          <span
            className={`h-2 w-2 rounded-full ${
              isApiConfigured ? "bg-emerald-400" : "bg-rose-400"
            }`}
            aria-hidden="true"
          />
          {isApiConfigured ? (
